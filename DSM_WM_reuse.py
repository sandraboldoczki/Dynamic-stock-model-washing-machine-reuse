...
#import os ## navigiert durch Ordner Ebenen
...
Pro_RunPath = Pro_ResultPath + now ## "/" pathlab creates a subfolder
Path(Pro_RunPath).mkdir(parents=True, exist_ok=True)## subfolder created
shutil.copy(Pro_ModelPath+scriptname,Pro_RunPath) ## source + target

##Read Input Data
Pro_DataFileName = '02_Input\\Input_DSMWM.xlsx'
Pro_DataFilePath = Pro_MainPath + Pro_DataFileName
Pro_DataFileWB = xlrd.open_workbook (Pro_DataFilePath)
...
#%%
    ...
    for y in range (0, Par_NoOfYears):#For all years
        for c in range (0, y+1): #Cohort c from first to last year       
                 for e in range (0, Par_NoOfEEC):
                    ...
                    if Par_RestDemand[y]< MaxReuse [y,y]: # If residual demand smaller than maximum reuse quota,then only reuse demanded products, rest goes to recycling
    ...
    print (now)
    dest_filename = Pro_ResultPath + 'Results DSM_Sc' + str(sc + 1) + '_' + str(now) +'.xlsx'
