...
import openpyxl as xl #read data files abd configs

#%%
"""Set Paths"""
# Load a local copy of the current ODYM branch:
# add ODYM module directory to system path, relative
##Path_Main = os.path.join('..', 'odym-openpyxl', 'modules')
##sys.path.insert(0, Path_Main)
# add ODYM module directory to system path, absolute
##sys.path.insert(0, os.path.join(os.getcwd(),'..', 'odym-openpyxl', 'modules'))
...
FileName_Script = 'DSM_WM_reuse_path.py'
Path_Results    = Path().resolve().parents[0] / 'Results'
Path.mkdir(Path_Results, exist_ok = True)
Path_Script     = Path() / FileName_Script

##Define Main Path
##Pro_MainPath   = 'M:\\02 - FORSCHUNG\\Forschung_Sandra\\11_MFA\\Dynamic stock model\\'
##Pro_ResultPath = Pro_MainPath + '01_Modell Python\\Results\\'
##Pro_ModelPath = Pro_MainPath + '01_Modell Python\\'

##define scriptname
#scriptname = "DSM_WM_update.py"

##save copy of code
#now=datetime.now().strftime("%Y_%m_%d__%H_%M")
#Pro_RunPath = Path_Results + now ## "/" durch pathlab wird Unterordner erstellt, now ist in nächster Ebene nach results Path
dtime       = datetime.now()
dtimestr    = dtime.strftime("%Y_%m_%d__%H_%M_%S")
Path_Run = Path_Results / dtimestr
Path(Path_Run).mkdir(parents=True, exist_ok=True)## Unterordner erstellt
...
#ProjectSpecs_ConFile = 'ODYM_Config_MaTrace-multi.xlsx'
#Model_Configpath     = os.path.join(Path_Data, ProjectSpecs_ConFile)
#Pro_DataFileWB     = xl.load_workbook(Path_Data, data_only = True)

#Pro_DataFileName = 'Input_DSMWM.xlsx'
#Path_Data = os.path.join(os.getcwd(),'..', 'docs', 'files')
#Path_DataFile = os.path_Data.join(os.getcwd(),'..', 'docs', 'files')
#Pro_DataFilePath = Path_Data + Pro_DataFileName
...
""" 1-Dimensional ausgelegt, ggf. später durch mehrere Dimensionen erweitern um verschiedene Lebenszeiten, Verteilungsfunktionen für den EEK bzw. Reuse annehmen)"""
#Par_Lifetime_New = np.zeros(Par_NoOfEEC)
#Par_Lifetime_Reuse = np.zeros(Par_NoOfEEC)
#Par_LTNew_Shape = np.zeros(Par_NoOfEEC)
#Par_LTReuse_Shape = np.zeros(Par_NoOfEEC)
...
#for m in range (0, Par_NoOfYears):
     ...
     #Par_PDF [n,m] = scipy.stats.weibull_min(Par_LTNew_Shape, 0, Par_Lifetime_New).cdf(n-m) - scipy.stats.weibull_min( Par_LTNew_Shape,0,Par_Lifetime_New).cdf(n-m-1)
...
#%%
        ...
        for c in range (0, y+1): #Cohort c from first to last year       
                 for e in range (0, Par_NoOfEEC):
                    ...
                    if c != 0: # year 0 determined prior to loop
                        Par_ToBeProduced [y] = max(Par_DemandbyYear [y] - Stock [y,:c,:].sum() - Stock_Reuse [y,:c,:].sum() - Fl_42 [y-1,:,:].sum(),0) #Berechnet zu produzierende
                    ...
                    EoL_Buffer[y,y,e] = Fl_23 [y,:,e].sum() #Alle Geräte, die EoL erreichen Landen in Puffer
                    Stock_Difference [y] = max((Par_DemandbyYear [y] - Stock [y,:c,:].sum() - Stock_Reuse [y,:c,:].sum() ),0) #Lücke im Bestand die gefüllt werden muss 
                    Par_RestDemand [y] = max(Stock_Difference[y], 0) #Restnachfrage ergibt sich daraus
                    
                    MaxReuse [y,y] = Par_Alpha [y,sc] * Fl_23[y,:,:].sum() #Maximale VzW-Quote, abhängig von Alpha
                    
                    Par_CheckAlpha [y] = Par_Alpha[y,sc]
                    ToSort [y,y] = MaxReuse [y,y] #Geräte, die noch sortiert werden müssen 
                    
                    if Par_RestDemand[y]< MaxReuse [y,y]: #Wenn Restnachfrage kleiner als maximale Reusequote, nur benötigte Wiederverwenden und Rest in Recycling schicken
                    ...
                    for n in range (0,y): #Separate Schleife für Sortierung (Grund unbekannt, aber geht sonst nicht)
                        for s in range (0, Par_NoOfEEC):
                            
                       
                                if EoL_Buffer [n,n,s] > 0:
                                        Reused [n,n,s] =  min (ToSort[n,n], EoL_Buffer[n,n,s]) #Minimum aus zu Sortierenden und noch zu sortierenden Geräten 
                                        Fl_34 [n,n,s] += Reused [n,n,s] #Reused wird VzW zugeteilt
                                        Fl_42 [n,n,s] = Fl_34 [n,n,s] #Verknüpfung Flow 42 und Fl 34
                                        EoL_Buffer [n,n,s] = EoL_Buffer [n,n,s] - Reused [n,n,s] #Wiederverwendete Geräte aus Puffer abgezogen
                                        Fl_35 [n,n,s] = EoL_Buffer [n,n,s] #Rest in Recycling
                                        EoL_Buffer [n,n,s] -= Fl_35 [n,n,s] #Recycling vom Buffer abziehen (sollte 0 sein am Ende ) 
                                        ToSort [n,n] -=  Reused [n,n,s] #Wiederverwendete Geräte aus Reused abziehen
                                       
                       
                    Total_Stock_New [y,e] = Stock [y,:,e].sum() #Summe über alle Kohorten pro Jahr
                    Total_Stock_Reuse [y,e] = Stock_Reuse [y,:,e].sum()
                    
                    Total_Reuse [c,e] = Fl_42[c,:,e].sum() #Summe Flow 42
                    Total_Recycling [c,e] = Fl_35[c,:,e].sum()+ fR_35[c,:,e].sum()
                    Total_Production[c,e] = Fl_12[c,:,e].sum() 
                    Total_EoL[c,e] = Fl_23 [c,:,e].sum()+ fR_23[c,:,e].sum() #Summe EoL.. Achtung! Enthält auch die Reusegeräte!
  ...
  #  Path_Run = Path_Results / dtimestr
   # Results = os.path.abspath(Path_Run)
    #dest_filename = Results + 'Results DSM_Sc' + str(sc + 1) + '_' + str(now) +'.xlsx'
    dest_filename = os.path.join(Path_Run, 'Results DSM_Sc' + str(sc + 1) + '_' + str(now) +'.xlsx')
   # dest_filename = 'Results DSM_Sc' + str(sc + 1) + '_' + str(now) +'.xlsx'
